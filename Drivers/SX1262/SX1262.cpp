...
#define digitalRead(pin) HAL_GPIO_ReadPin(pin##_GPIO_Port, pin##_Pin)
#define digitalWrite(pin, state) HAL_GPIO_WritePin(pin##_GPIO_Port, pin##_Pin, state)
...
SX1262::SX1262(uint16_t nss,      GPIO_TypeDef *nss_gpio,
               ...
               SPI_HandleTypeDef *hspi)
{
    // Assign pin numbers
    SPI_NSS_Pin     = nss;
    RF_NRESET_Pin   = nreset;
    RF_BUSY_Pin     = busy;
    RF_DIO1_Pin     = dio1;

    // Assign GPIO port pointers
    SPI_NSS_GPIO_Port     = nss_gpio;
    RF_NRESET_GPIO_Port   = nreset_gpio;
    RF_BUSY_GPIO_Port     = busy_gpio;
    RF_DIO1_GPIO_Port     = dio1_gpio;

    // Assign SPI handle
    this->hspi = hspi;

    // Optionally allocate and initialize lora_para_pt if it's a pointer
//    if (lora_para_pt == nullptr)
//    {
//        lora_para_pt = new LoraParams();  // <-- Ensure LoraParams is defined and included
//    }

    // Initialize parameters (example defaults)
//    lora_para_pt->rf_freq      = 868000000;
//    lora_para_pt->tx_power     = 14;
//    lora_para_pt->lora_sf      = 7;
//    lora_para_pt->band_width   = 125;
//    lora_para_pt->code_rate    = 1;
//    lora_para_pt->payload_size = 16;
}

void SX1262::SPI_Init(void)
{
// This is supposed to be configured in ioc
...
//
//	pinMode(RF_BUSY, INPUT);
//	pinMode(RF_DIO1, INPUT);
}
...
{
	digitalWrite(RF_NRESET, LOW);
	delay(2);		//more thena 100us, delay 2ms
...
uint8_t SX1262::spi_rw(uint8_t value_w)
{
	...
	HAL_SPI_TransmitReceive(hspi, &value_w, &value_r, 1, HAL_MAX_DELAY);
...
void SX1262::SetPacketParams(uint8_t payload_len)
{
	...
	spi_rw(0x01);//HeaderType 0:Variable,explicit 1:Fixed,implicit
	//spi_rw(0x01);
...
void SX1262::SX1262_Config(void)
{
	...
	SetTxParams(power_temp,SET_RAMP_800U);	//set power and ramp_time
...
void SX1262::setSyncWord(uint16_t syncWord)
{
    uint8_t buf[2];

    // Default case: 2-byte sync word
    buf[0] = syncWord >> 8;
    buf[1] = syncWord & 0xFF;

    // Optional LoRaWAN-style 1-byte sync word mapping
    if (syncWord <= 0xFF) {
        buf[0] = (syncWord & 0xF0) | 0x04;
        buf[1] = (syncWord << 4) | 0x04;
    }

    // Use your HAL-compatible write function
